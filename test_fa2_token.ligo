  ...
  block {
    ...
    function make_transfer(var s : fa2_storage; const transfer_dst : transfer_destination) : fa2_storage is
      block {
        ...
        (* Check permissions *)
        require(params.from_ = Tezos.sender or src_account.permits contains Tezos.sender, "Errors_FA2_notOperator");


        // // (* Token id check *)
        // require(transfer_dst.token_id < s.last_token_id, "Errors_FA2_undefined");
        ...
        src_account.balances[transfer_dst.token_id] := get_nat_or_fail(
          ...
          "Errors_FA2_lowBalance"
        );

        (* Update storage *)
        s.ledger[params.from_] := src_account;
        ...
        (* Update storage *)
        s.ledger[transfer_dst.to_] := dst_account;
    } with s
...
function iterate_update_operators(var s : fa2_storage; const params : update_operator_param) : fa2_storage is
  block {
    ...
    | Add_operator(param) -> block {
      (* Check an owner *)
      require(Tezos.sender = param.owner, "Errors_FA2_notOwner");
      ...
      (* Update storage *)
      s.ledger[param.owner] := src_account;
    }
  ...
  block {
    ...
    function make_mint(
      ...
      const param       : asset_param)
                        : fa2_storage is
      block {
        // require(param.token_id < s.last_token_id, "Errors_FA2_undefined");
        ...
        (* Update storage *)
        s.ledger[param.receiver] := dst_account;
...
function create_token(
  ...
  var s                 : fa2_storage)
                        : fa2_storage is
  block {
    require(s.admin = Tezos.sender, "Errors_FA2_notAdmin");
